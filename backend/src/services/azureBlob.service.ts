...
function getContainerClient(): Promise<ContainerClient> {
  if (!containerClientPromise) {
    containerClientPromise = (async () => {
      const { azureStorageAccountName, azureStorageAccountKey, azureStorageContainerName } = env;

      if (!azureStorageAccountName || !azureStorageAccountKey || !azureStorageContainerName) {
        console.error("[AzureBlob] ❌ Missing Azure credentials. Ensure AZURE_STORAGE_ACCOUNT_NAME, AZURE_STORAGE_ACCOUNT_KEY and AZURE_STORAGE_CONTAINER_NAME are set.");
        throw new AppError(
          500,
      ...
      console.log(`[AzureBlob] 🔌 Initialising container client → account="${azureStorageAccountName}" container="${azureStorageContainerName}"`);


      const credential = new StorageSharedKeyCredential(azureStorageAccountName, azureStorageAccountKey);
      ...
      console.log(`[AzureBlob] ✅ Container client ready → container="${azureStorageContainerName}"`);
 ...
 */
export function extractBlobName(urlOrBlobName: string | undefined | null): string | undefined {
...
export async function uploadBlob(
  ...
  console.log(`[AzureBlob] ⬆️  uploadBlob START → blobName="${blobName}" contentType="${contentType}" size=${buffer.length}B`);
  try {
    ...
    console.log(`[AzureBlob] ✅ uploadBlob SUCCESS → blobName="${blobName}" url="${url}"`);
    return { url, blobName };
  } catch (err: any) {
    console.error(`[AzureBlob] ❌ uploadBlob FAILED → blobName="${blobName}" error="${err?.message || err}"`);
    if (err instanceof AppError) throw err;
    throw new AppError(502, `Azure Blob upload failed: ${err?.message || "Unknown error"}`);
  }
}
...
export async function replaceBlob(
  ...
  console.log(`[AzureBlob] 🔄 replaceBlob → delegating to uploadBlob for newBlobName="${newBlobName}"`);
 ...
 */
export async function deleteBlob(urlOrBlobName: string | undefined | null): Promise<void> {
  const blobName = extractBlobName(urlOrBlobName || undefined);
  if (!blobName) {
    console.warn(`[AzureBlob] ⚠️  deleteBlob called with empty/invalid reference → input="${urlOrBlobName}" (skipping)`);
    return;
  }
  ...
  try {
    ...
    if (result.succeeded) {
      console.log(`[AzureBlob] ✅ deleteBlob SUCCESS → blobName="${blobName}" (blob existed and was deleted)`);
    } else {
      console.log(`[AzureBlob] ℹ️  deleteBlob NOOP → blobName="${blobName}" (blob did not exist, nothing deleted)`);
    }
  } catch (err: any) {
    console.error(`[AzureBlob] ❌ deleteBlob FAILED → blobName="${blobName}" error="${err?.message || err}"`);
    if (err instanceof AppError) throw err;
    throw new AppError(502, `Azure Blob delete failed: ${err?.message || "Unknown error"}`);
  }
}
